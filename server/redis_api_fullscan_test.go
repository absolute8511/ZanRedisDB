...
func TestKVFullScanCrossTable(t *testing.T) {
	...
	if ay, err := goredis.Values(c.Do("FULLSCAN", "default:testscanmerge:", "KV", "COUNT", 1000)); err != nil {
		t.Fatal(err)
	} else if len(ay) != 2 {
		t.Fatal(len(ay))
	} else {
	...
	if ay, err := goredis.Values(c.Do("FULLSCAN", "default:testscanmerge1:", "KV", "COUNT", 1000)); err != nil {
		t.Fatal(err)
	} else if len(ay) != 2 {
		t.Fatal(len(ay))
	} else {
